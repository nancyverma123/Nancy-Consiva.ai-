...
- If the context describes a manual action (e.g. clicking a button, selecting an option),
  do not describe it as automatic, or add any mechanism, condition, or trigger not stated
  in the context — even if it sounds plausible or is common in similar products.

- When explaining how a feature works, only state the mechanism explicitly described in
  the context. If the context doesn't explain the mechanism, describe the feature without
...
short, natural spoken sentences (under 60 words in total). Do not use markdown, bullet points,
tables, headings, emojis, or URLs. If there is more detail, briefly offer to explain further.
"""
...
def _translate_to_english(query: str, language_code: str) -> str:
    """Translate non-English queries before embedding them for retrieval."""
...
def retrieve(
    ...
    """Retrieve relevant Consiva knowledge from Pinecone."""
    ...
    # Pinecone returns a QueryResponse object.
    result = index.query(
        vector=query_vector,
        top_k=top_k,
    ...
    print(
        f"Query: {query!r} "
        f"threshold={settings.rag_relevance_threshold}"
    )

    # FIX:
    # Pinecone QueryResponse uses .matches rather than .get("matches").
...
def generate_answer(
    ...
    # Translate only for retrieval.
    retrieval_query = _translate_to_english(
        user_message,
        language_code,
    )

    # Retrieve relevant Consiva information.
    ...
    messages.append(
        {
            ...
            "content": (
                f"<{boundary}-context>\n"
                f"{neutralize_boundaries(context_block)}\n"
                f"</{boundary}-context>\n\n"
                f"<{boundary}-question>\n"
                f"{neutralize_boundaries(user_message)}\n"
                f"</{boundary}-question>"
            ),
        }
    )
    ...
    completion = client.chat.completions.create(
        ...
        messages=messages,
        temperature=0.2,
        # gpt-oss models count reasoning tokens against max_tokens,
        # so leave headroom.
        max_tokens=1500,
        extra_body={
    ...
    # Remove internal fallback markers before returning the answer.
    for marker in (
        OUT_OF_SCOPE_MARKER,
        NO_CONTEXT_MARKER,
    ):
